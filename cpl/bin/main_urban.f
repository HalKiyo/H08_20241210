...
c      parameter        (n0l=2304)
      ...
      parameter        (n0swim=71)
      parameter        (n0ram=19)
      parameter        (n0rec=40)        !! # grids to deliver river water
...
c hydrological parameter optimization
      real              r1soildepth2(n0l)
      real              r1cd2(n0l)
      real              r1gamma2(n0l)
      real              r1tau2(n0l)
      real              r0soildepth_cropland
      real              r0cd_cropland
      real              r0gamma_cropland
      real              r0tau_cropland
      data              r0soildepth_cropland/1.0/
      data              r0cd_cropland/0.003/
      data              r0gamma_cropland/2.0/
      data              r0tau_cropland/100.0/
c end
c namelist (lnd)
...
c in (set, withdrawal)
      character*128     c0optnnbs    !! option (non-local -renew surf)
      character*128     c0optnnbg    !! option (non-local -renew gw)
...
c local (dam)
      ...
      real              r1krls(n0l)
c local (irrigation)
      integer           i2flgcul(n0l,n0c)       !! flgcul is not mosaic-wise
      integer           i2flgculkiller(n0l,n0c) !! flgculkiller reset flgcul 
      integer           i2flgirg(n0l,n0c)       !! after the mosaic-loop.
...
c urban start
      integer           i0lctydrn
      real              r1ctydrn(n0l) !! drainage destination (urban sewage)
...
c     namelist
     ...
     $     c0damalc,      c0optnnbs,     c0optnnbg,     c0opthvsdoy,
     $     c1pltdoy,      c1hvsdoy,      c1crptyp,      c0lndara,
     $     c1arafrc,      c0optriv,
...
c
cvar      do i0m=1,n0m
cvar        call read_binary(n0l,c1arafrc(i0m),r1tmp)
cvar        do i0l=1,n0l
cvar          r2arafrc(i0l,i0m)=r1tmp(i0l)
cvar        end do
cvar      end do
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      do i0c=1,n0c
        call read_binary(n0l,c1pltdoy(i0c),r1tmp)
        do i0l=1,n0l
          i2pltdoy(i0l,i0c)=int(r1tmp(i0l))
        end do
      end do
c
      do i0c=1,n0c
        call read_binary(n0l,c1hvsdoy(i0c),r1tmp)
        do i0l=1,n0l
          i2hvsdoy(i0l,i0c)=int(r1tmp(i0l))
        end do
      end do
c
      do i0c=1,n0c
        call read_binary(n0l,c1crptyp(i0c),r1tmp)
        do i0l=1,n0l
          i2crptyp(i0l,i0c)=int(r1tmp(i0l))
        end do
      end do
      call read_binary(n0l,c0lndara,r1lndara)
d     write(*,*) 'main: --- Read fixed fields -----------------------'
d     write(*,*) 'main: i2pltdoy: ',i2pltdoy(i0ldbg,1)
d     write(*,*) 'main: i2hvsdoy: ',i2hvsdoy(i0ldbg,1)
d     write(*,*) 'main: i2crptyp: ',i2crptyp(i0ldbg,1)
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc 
cvar      if(c0damid_.ne.'NO')then
cvar        call read_binary(n0l,c0damid_,r1tmp)
cvar        do i0l=1,n0l
cvar          i1damid_(i0l)=int(r1tmp(i0l))
cvar        end do
cvar      else
cvar        i1damid_=0
cvar      end if
c
cvar      if(c0damprp.ne.'NO')then
cvar        call read_binary(n0l,c0damprp,r1tmp)
cvar        do i0l=1,n0l
cvar          i1damprp(i0l)=int(r1tmp(i0l))
cvar        end do
cvar      else
cvar        i1damprp=0
cvar      end if
...
c
cvar      if(c0damcap.ne.'NO')then
cvar        call read_binary(n0l,c0damcap,r1damcap)
cvar      else
cvar        r1damcap=0.0
cvar      end if
c
cvar      if(c0msrcap.ne.'NO')then
cvar        call read_binary(n0l,c0msrcap,r1msrcap)
cvar      else
cvar        r1msrcap=0.0
cvar      end if
c
cvar      if(c0msrafc.ne.'NO')then
cvar        call read_binary(n0l,c0msrafc,r1msrafc)
cvar      else
cvar        r1msrafc=1.0
cvar      end if
c 
cvar      if(c0demagrfix.ne.'NO')then
cvar        call read_binary(n0l,c0demagrfix,r1demagrfix)
cvar      else
cvar        r1demagrfix=0.0
cvar      end if
c
cvar      if(c0demind.ne.'NO')then
cvar      call read_binary(n0l,c0demind,r1demind)
cvar      else
cvar        r1demind=0.0
cvar      end if
c
cvar      if(c0demdom.ne.'NO')then
cvar      call read_binary(n0l,c0demdom,r1demdom)
cvar      else
cvar        r1demdom=0.0
cvar      end if
c 
...
c
        if(c0demind.ne.'NO')then
          call read_result(
     $         n0l,c0demind,i0year,0,
     $         0,0,i0secint,r1demind)
        else
          r1demind=0.0
        end if
c     
        if(c0demdom.ne.'NO')then
          call read_result(
     $         n0l,c0demdom,i0year,0,
     $         0,0,i0secint,r1demdom)
        else
          r1demdom=0.0
        end if
c
...
c
        if(c0damcap.ne.'NO')then
          call read_result(
     $         n0l,c0damcap,i0year,0,
     $         0,0,i0secint,r1damcap)
        else
          r1damcap=0.0
        end if
c     
...
c     
        if(c0msrcap.ne.'NO')then
          call read_result(
     $         n0l,c0msrcap,i0year,0,
     $         0,0,i0secint,r1msrcap)
        else
          r1msrcap=0.0
        end if
c
        if(c0msrafc.ne.'NO')then
          call read_result(
     $         n0l,c0msrafc,i0year,0,
     $         0,0,i0secint,r1msrafc)
        else
          r1msrafc=0.0
        end if
c
        if(c0demagrfix.ne.'NO')then
          call read_result(
     $         n0l,c0demagrfix,i0year,0,
     $         0,0,i0secint,r1demagrfix)
        else
          r1demagrfix=0.0
        end if
cvar end
...
c for parameter optimization
                  if(i0m.eq.n0m)then
                    do i0l=1,n0l
                      r1soildepth2(i0l)=r1soildepth(i0l)
                    end do
                  else
                    do i0l=1,n0l
                      r1soildepth2(i0l)=r0soildepth_cropland
                    end do
                  end if
c
                  call calc_irgapp(
     ...
     $                 c1optlnduse(i0m),
     $                 i2crptyp,    r1soildepth2, r1w_fieldcap,
...
c
                if(i0m.eq.n0m)then
                  ...
                  do i0l=1,n0l
                    r1gamma2(i0l)=r1gamma(i0l)
                  end do
                  do i0l=1,n0l
                    r1tau2(i0l)=r1tau(i0l)
                  end do
                else
                  do i0l=1,n0l
                    r1soildepth2(i0l)=r0soildepth_cropland
                  ...
                  do i0l=1,n0l
                    r1cd2(i0l)=r0cd_cropland
                  end do
                  do i0l=1,n0l
                    r1gamma2(i0l)=r0gamma_cropland
                  end do
                  do i0l=1,n0l
                    r1tau2(i0l)=r0tau_cropland
                  end do
                end if
c
                call calc_leakyb(
...
c debug
                do i0l=1,n0l
                  if(r1qsb(i0l).lt.0.0)then
                    write(*,*) 'main: negative baseflow at',i0l,
     $                   r1qsb(i0l),r1soilmoist(i0l),r1evap(i0l)
                    stop
                  end if
                end do
c
...
c
              ...
              do i0l=1,n0l
c urban start
                i0lctydrn=int(r1ctydrn(i0l))
                if(r1demind(i0l).ne.p0mis)then
c urban                  r1rtfind(i0l)=r1supind(i0l)*(1.0-r1indeff(i0l))
c     urban                  r1supind(i0l)=r1supind(i0l)*r1indeff(i0l)
c debug
                   write(*,*) i0l,i0lctydrn
                  r1rtfind(i0lctydrn)
     &            =r1rtfind(i0lctydrn)+r1supind(i0l)*(1.0-r1indeff(i0l))
...
c urban start
                i0lctydrn=int(r1ctydrn(i0l))
                if(r1demdom(i0l).ne.p0mis)then
c urban                  r1rtfdom(i0l)=r1supdom(i0l)*(1.0-r1domeff(i0l))
c urban                  r1supdom(i0l)=r1supdom(i0l)*r1domeff(i0l)
                   r1rtfdom(i0lctydrn)
     &            =r1rtfdom(i0lctydrn)+r1supdom(i0l)*(1.0-r1domeff(i0l))
...
c     debug hanasaki
            else if(i0m.eq.n0m-1)then
               write(*,*) 'main: gwb problem ascend-1 at ',i0l
               write(*,*) 'arafrc(i0l,i0m) ',r2arafrc(i0l,i0m)
               write(*,*) 'arafrc(i0l,i0m+1) ',r2arafrc(i0l,i0m+1)
               write(*,*) 'rgw(i0l,0,i0m) ',r3rgw(i0l,0,i0m)
     ...
     $               +min(0.0,r3rgw(i0l,0,i0m))
     $              *r2arafrc(i0l,i0m)/r2arafrc(i0l,i0m-1)
c debug hananasaki (2024/02/23)
            else if(i0m.eq.2)then
               write(*,*) 'main: gwb problem descend-1 at ',i0l
               write(*,*) 'arafrc(i0l,i0m) ',r2arafrc(i0l,i0m)
               write(*,*) 'arafrc(i0l,i0m-1) ',r2arafrc(i0l,i0m-1)
               write(*,*) 'rgw(i0l,0,i0m) ',r3rgw(i0l,0,i0m)
               write(*,*) 'rgw(i0l,0,i0m-1) ',r3rgw(i0l,0,i0m-1)
               write(*,*) '-----'    
               if(r3rgw(i0l,0,i0m).lt.0.0)then
                 r3rgw(i0l,0,i0m)=0.0
               end if
               continue          
...
c debug hananasaki (2024/02/23)
               ...
               write(*,*) 'main: gwb problem descend-2 at ',i0l
               write(*,*) 'arafrc(i0l,i0m) ',r2arafrc(i0l,i0m)
               write(*,*) 'arafrc(i0l,i0m-2) ',r2arafrc(i0l,i0m-2)
               write(*,*) 'rgw(i0l,0,i0m) ',r3rgw(i0l,0,i0m)
               write(*,*) 'rgw(i0l,0,i0m-2) ',r3rgw(i0l,0,i0m-2)
               write(*,*) '-----'               
               if(r3rgw(i0l,0,i0m).lt.0.0)then
                 r3rgw(i0l,0,i0m)=0.0
               end if
...
d              write(*,*) 'main: r3rgw(1):',r3rgw(i0ldbg,0,1)
d              write(*,*) 'main: r3rgw(2):',r3rgw(i0ldbg,0,2)
d              write(*,*) 'main: r3rgw(3):',r3rgw(i0ldbg,0,3)
d              write(*,*) 'main: r3rgw(4):',r3rgw(i0ldbg,0,4)
...
c
              if(i0sec.eq.n0secday)then
                ...
                do i0m=1,n0m
c
                  if(c1optlnduse(i0m).eq.'dci'.or.
     $               c1optlnduse(i0m).eq.'dcr')then
                    do i0l=1,n0l
...
c
                    if(i1flgend(i0l).eq.1)then
                      if(i1flg2nd(i0l).eq.1)then
                        i2flgculkiller(i0l,2)=1
                      else
                        i2flgculkiller(i0l,1)=1
                      end if
                      r1cwd(i0l)=0.0
                      r1cws(i0l)=0.0
                      r1cwsgrn(i0l)=0.0
...
c
                do i0l=1,n0l
                  if(i2flgculkiller(i0l,1).eq.1)then
                    i2flgcul(i0l,1)=0
                    i2flgculkiller(i0l,1)=0
                  end if
                  if(i2flgculkiller(i0l,2).eq.1)then
                    i2flgcul(i0l,2)=0
                    i2flgculkiller(i0l,2)=0
                  end if
                end do
c
              end if
